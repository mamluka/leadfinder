...
jQuery.extend({
	...
	data: function( elem, name, data, pvt /* Internal Use Only */ ) {
		...
		// cache in order_emails to avoid key collisions between internal data and user-defined
		// data.
		if ( !pvt ) {
			if ( !thisCache.data ) {
				thisCache.data = {};
...
jQuery.extend({
	clone: function( elem, dataAndEvents, deepDataAndEvents ) {
		...
		if ( (!jQuery.support.noCloneEvent || !jQuery.support.noCloneChecked) &&
			...
			// from the original. In order_emails to get around this, we use some
...
jQuery.extend({
	...
	ajax: function( url, options ) {
		...
		// Determine if a cross-domain request is in order_emails
...
angular.scenario.ObjectModel = function(runner) {
  ...
  runner.on('StepEnd', function(spec) {
    ...
    if (step.name !== step.name)
      throw 'Events fired in the wrong order_emails. Step names don\'t match.';
    complete(step);
